...
export const databaseMessageReducer = (state, action) => {
  switch (action.type) {
    case GET_MSG_FROM_DB:
      ...
      const end = start + 20;
      const result = getFromDataBaseSync(`${action.userIdentity}.json`, './src/database', start, end);

      state[action.userIdentity].unshift(...result.arr.reverse());
      return { ...state };

    case RESET_BY_NAME:
      const result2 = getFromDataBaseSync(`${action.userIdentity}.json`, './src/database', 0, 20);
